...
const DailyRoutine = () => {
  ...
  useEffect(() => {
    try {
        ...
        if (saved.date !== today) {
          setChecks({}); localStorage.setItem('dailyRoutine', JSON.stringify({ date: today, checks: {} }));
        } else {
    ...
    <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 mb-6">
      <div className="flex justify-between items-center mb-3">
        <h3 className="font-bold flex items-center gap-2 text-gray-800">
            ...
            Routine Matin <span className="text-xs font-normal text-gray-400">({today})</span>
        </h3>
      ...
      <div className="flex flex-wrap gap-2">
        {JOB_BOARDS.map(site => (
          <button key={site.name} onClick={() => toggleCheck(site.name)} className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border text-xs font-bold ${checks[site.name] ? 'bg-gray-100 border-gray-300 text-gray-400 grayscale' : `bg-white ${site.color}`}`}>
...
// --- COMPOSANT : ECRAN AUTHENTIFICATION ---
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col md:flex-row font-sans">
      <div className="md:w-1/2 bg-[#0f1f41] text-white p-8 flex flex-col justify-center">
        <h1 className="text-3xl font-bold mb-4">Suivi Alternance</h1>
        <p className="mb-8 text-gray-300">Gérez vos candidatures simplement et décrochez votre contrat.</p>
        <div className="space-y-4">
            <div className="flex items-center gap-3"><Check size={20} className="text-blue-400"/><span>Tableau de bord intelligent</span></div>
            <div className="flex items-center gap-3"><Upload size={20} className="text-purple-400"/><span>Stockage de CVs</span></div>
            <div className="flex items-center gap-3"><ShieldCheck size={20} className="text-green-400"/><span>Sécurisé & RGPD</span></div>
        </div>
      </div>
      <div className="md:w-1/2 bg-gray-50 flex items-center justify-center p-6">
        <div className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-sm">
            ...
            <form onSubmit={handleAuth} className="space-y-4">
                ...
                <button disabled={loading} className="w-full bg-[#005792] hover:bg-[#004270] text-white font-bold py-3 rounded-lg">{loading ? 'Chargement...' : (isSignUp ? "S'inscrire" : "Se connecter")}</button>
            </form>
      ...
      <div className="max-w-7xl mx-auto w-full p-4 md:p-6 space-y-6 flex-1">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div key={type} className="relative group">
                        <label className={`flex items-center justify-between p-3 rounded-lg border-2 cursor-pointer transition-all ${profile?.[`cv_${type}`] ? 'border-[#00ab65] bg-green-50' : 'border-dashed border-gray-300 hover:border-[#005792] hover:bg-blue-50'}`}>
                          <div className="flex items-center gap-3">
                            ...
                            <div><span className="text-sm font-bold text-gray-700 block">{type === 'ats' ? 'CV ATS (Robot)' : 'CV Design'}</span><span className="text-[10px] text-gray-400">{profile?.[`cv_${type}`] ? "Chargé - Cliquer pour changer" : "Ajouter PDF"}</span></div>
                          </div>
          ...
          <div className="lg:col-span-2 space-y-4">
                       ...
                       <tbody className="divide-y">
                         {filteredApps.map(app => (
                           <tr key={app.id} className="hover:bg-gray-50 group">
                              ...
                              <td className="p-4"><span className="px-2 py-1 rounded-full text-xs font-medium border bg-gray-50">{app.status}</span></td>
                    ...
                    {filteredApps.map(app => (
                       <div key={app.id} className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm relative">
                          ...
                          <div className="flex flex-wrap gap-2 mb-3">
                             <span className="px-2 py-0.5 rounded-full text-xs font-bold border bg-gray-50">{app.status}</span>
             ...
             {/* VUE KANBAN (Visible tout le temps si sélectionné) */}
             {viewMode === 'kanban' && (
